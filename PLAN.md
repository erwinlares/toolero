...
- read_clean\_*() function family. read_clean_csv() as the first member,
  already in the package. Planned additions: read_clean_excel() – same
  pattern via readxl Shared conventions: janitor::clean_names() as core,
  optional na_handling, type_inference, verbose arguments, returning
  tibbles. The drop_na argument for read_clean_csv() accepts TRUE or
  c(col1, col2) for column-specific dropping with cli feedback. Prefix
  convention is read_clean\_*().
