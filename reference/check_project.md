...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpBaQ2VO/file4e9fe4141c1 does not exist.
# }
...
#> ✔ Created /tmp/RtmpBaQ2VO/my-config.yml
#> ℹ Edit /tmp/RtmpBaQ2VO/my-config.yml to define your custom folder structure,
#>   then pass it to `init_project()` via `config =
#>   "/tmp/RtmpBaQ2VO/my-config.yml"`.
...
#> Error in check_project(path = project_dir, config = config_path): Directory /tmp/RtmpBaQ2VO/file4e9f47199406 does not exist.
# }
...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpBaQ2VO/file4e9f21e76df8 does not exist.
# }
```
