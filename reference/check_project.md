...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpLTsR81/file4e992b0bfb08 does not exist.
# }
...
#> ✔ Created /tmp/RtmpLTsR81/my-config.yml
#> ℹ Edit /tmp/RtmpLTsR81/my-config.yml to define your custom folder structure,
#>   then pass it to `init_project()` via `config =
#>   "/tmp/RtmpLTsR81/my-config.yml"`.
...
#> Error in check_project(path = project_dir, config = config_path): Directory /tmp/RtmpLTsR81/file4e9975a612ef does not exist.
# }
...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpLTsR81/file4e995e1538ec does not exist.
# }
```
