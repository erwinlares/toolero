...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpWoG8Ew/file4e9bfafed76 does not exist.
# }
...
#> ✔ Created /tmp/RtmpWoG8Ew/my-config.yml
#> ℹ Edit /tmp/RtmpWoG8Ew/my-config.yml to define your custom folder structure,
#>   then pass it to `init_project()` via `config =
#>   "/tmp/RtmpWoG8Ew/my-config.yml"`.
...
#> Error in check_project(path = project_dir, config = config_path): Directory /tmp/RtmpWoG8Ew/file4e9b2549ed87 does not exist.
# }
...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpWoG8Ew/file4e9b4fea64f7 does not exist.
# }
```
