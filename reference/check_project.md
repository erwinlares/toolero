...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpT5WO3x/file4d6738c19c1b does not exist.
# }
...
#> ✔ Created /tmp/RtmpT5WO3x/my-config.yml
#> ℹ Edit /tmp/RtmpT5WO3x/my-config.yml to define your custom folder structure,
#>   then pass it to `init_project()` via `config =
#>   "/tmp/RtmpT5WO3x/my-config.yml"`.
...
#> Error in check_project(path = project_dir, config = config_path): Directory /tmp/RtmpT5WO3x/file4d67499279b5 does not exist.
# }
...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpT5WO3x/file4d67cd8ae72 does not exist.
# }
```
