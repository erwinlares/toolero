...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpwfS68b/file50c47fd7232f does not exist.
# }
...
#> ✔ Created /tmp/RtmpwfS68b/my-config.yml
#> ℹ Edit /tmp/RtmpwfS68b/my-config.yml to define your custom folder structure,
#>   then pass it to `init_project()` via `config =
#>   "/tmp/RtmpwfS68b/my-config.yml"`.
...
#> Error in check_project(path = project_dir, config = config_path): Directory /tmp/RtmpwfS68b/file50c46803cce3 does not exist.
# }
...
#> Error in check_project(path = project_dir): Directory /tmp/RtmpwfS68b/file50c44829dcd4 does not exist.
# }
```
