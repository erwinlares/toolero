...
#> ✔ Written "Adelie" (2 rows) to /tmp/RtmpZBWMWX/adelie.csv
#> ✔ Written "Gentoo" (1 rows) to /tmp/RtmpZBWMWX/gentoo.csv
...
write_by_group(data2, group_col = c("species", "sex"),
               output_dir = tempdir(), manifest = TRUE)
#> ✔ Written "Adelie | female" (1 rows) to /tmp/RtmpZBWMWX/adelie--female.csv
#> ✔ Written "Adelie | male" (1 rows) to /tmp/RtmpZBWMWX/adelie--male.csv
#> ✔ Written "Gentoo | male" (1 rows) to /tmp/RtmpZBWMWX/gentoo--male.csv
#> ✔ Manifest written to /tmp/RtmpZBWMWX/manifest.csv
# }
```
