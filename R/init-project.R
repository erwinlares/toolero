...
#' }
init_project <- function(file_path, use_renv = TRUE, use_git = TRUE, extra_folders = NULL, open = TRUE) {
    ...
    purrr::walk(standard_folders, \(folder) fs::dir_create(glue::glue("{file_path}/{folder}")))

    # 3. create any extra folders
    if (!is.null(extra_folders)) {
        purrr::walk(extra_folders, \(folder) fs::dir_create(glue::glue("{file_path}/{folder}")))
    }
